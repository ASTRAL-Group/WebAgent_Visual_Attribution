...
"""
NPR 坐标计算脚本
专门负责从HTML中提取目标新闻文章的坐标

功能：
1. 使用Playwright从HTML中提取目标新闻文章的精确坐标
...
class NPRCoordinateCalculator:
    ...
    def __init__(self, input_dir: str, output_dir: str):
        ...
        print(f"🚀 NPR 坐标计算器已初始化")
        print(f"📁 输入目录: {self.input_dir}")
        print(f"📁 输出目录: {self.output_dir}")
        print(f"🎯 目标: 目标新闻文章 (article[data-target-card=\"true\"])")
    ...
    def extract_html_coordinates(self, html_file_path: str) -> Optional[Dict]:
        """
        从HTML中提取目标新闻文章的精确坐标（使用Playwright确保与截图一致）
        """
        ...
        try:
            ...
            with sync_playwright() as p:
                ...
                # 方法1: 查找带有 data-target-card="true" 的目标文章（最高优先级）
                try:
                    target_article = page.locator('article[data-target-card="true"]').first
                    if target_article.count() > 0:
                        bbox = target_article.bounding_box()
                        if bbox and bbox['width'] > 100 and bbox['height'] > 50:
                            # 获取文章标题作为匹配文本
                            try:
                                title_element = target_article.locator('.title, h3.title, .teaser').first
                                matched_text = title_element.text_content()[:100] if title_element.count() > 0 else "Target News Article"
                            except:
                                matched_text = "Target News Article"
                            ...
                            coordinates = {
                                ...
                                'confidence': 98,
                                'detection_method': 'target_article_selector',
                                'matched_text': matched_text
                            }
                            detection_method = "target_article_selector"
                except Exception as e:
                    self.logger.warning(f"目标文章选择器查找失败: {e}")
                
                # 方法2: 查找包含 data-target-card="true" 的 .bucketwrap 容器
                if not coordinates:
                    try:
                        target_container = page.locator('.bucketwrap:has(article[data-target-card="true"])').first
                        if target_container.count() > 0:
                            bbox = target_container.bounding_box()
                            if bbox and bbox['width'] > 100 and bbox['height'] > 50:
                                try:
                                    title_element = target_container.locator('.title, h3.title, .teaser').first
                                    matched_text = title_element.text_content()[:100] if title_element.count() > 0 else "Target News Container"
                                except:
                                    matched_text = "Target News Container"
                                ...
                                coordinates = {
                                    ...
                                    'center_y': int(bbox['y'] + bbox['height'] / 2),
                                    'confidence': 95,
                                    'detection_method': 'target_container_selector',
                                    'matched_text': matched_text
                                }
                                detection_method = "target_container_selector"
                    except Exception as e:
                        self.logger.warning(f"目标容器选择器查找失败: {e}")
                
                # 方法3: 查找第一个 article 元素（作为后备方法）
                if not coordinates:
                    try:
                        first_article = page.locator('#main-section article, #contentWrap article').first
                        if first_article.count() > 0:
                            bbox = first_article.bounding_box()
                            if bbox and bbox['width'] > 100 and bbox['height'] > 50:
                                try:
                                    title_element = first_article.locator('.title, h3.title, .teaser').first
                                    matched_text = title_element.text_content()[:100] if title_element.count() > 0 else "First News Article"
                                except:
                                    matched_text = "First News Article"
                                ...
                                coordinates = {
                                    ...
                                    'center_y': int(bbox['y'] + bbox['height'] / 2),
                                    'confidence': 90,
                                    'detection_method': 'first_article_selector',
                                    'matched_text': matched_text
                                }
                                detection_method = "first_article_selector"
                    except Exception as e:
                        self.logger.warning(f"第一个文章选择器查找失败: {e}")
                
                # 方法4: 查找第一个 .bucketwrap.promocard 容器
                if not coordinates:
                    try:
                        first_promocard = page.locator('.bucketwrap.promocard').first
                        if first_promocard.count() > 0:
                            bbox = first_promocard.bounding_box()
                            if bbox and bbox['width'] > 100 and bbox['height'] > 50:
                                try:
                                    title_element = first_promocard.locator('.title, h3.title, .teaser').first
                                    matched_text = title_element.text_content()[:100] if title_element.count() > 0 else "First Promo Card"
                                except:
                                    matched_text = "First Promo Card"
                                ...
                                coordinates = {
                                    ...
                                    'center_y': int(bbox['y'] + bbox['height'] / 2),
                                    'confidence': 85,
                                    'detection_method': 'first_promocard_selector',
                                    'matched_text': matched_text
                                }
                                detection_method = "first_promocard_selector"
                    except Exception as e:
                        self.logger.warning(f"第一个promocard选择器查找失败: {e}")
                
                # 方法5: 查找包含标题的第一个元素
                if not coordinates:
                    try:
                        # 查找包含标题的容器
                        article_with_title = page.locator('.bucketwrap:has(.title)').first
                        if article_with_title.count() > 0:
                            bbox = article_with_title.bounding_box()
                            if bbox and bbox['width'] > 100 and bbox['height'] > 50:
                                try:
                                    title_element = article_with_title.locator('.title, h3.title').first
                                    matched_text = title_element.text_content()[:100] if title_element.count() > 0 else "Article with Title"
                                except:
                                    matched_text = "Article with Title"
                                ...
                                coordinates = {
                                    ...
                                    'center_y': int(bbox['y'] + bbox['height'] / 2),
                                    'confidence': 80,
                                    'detection_method': 'article_with_title',
                                    'matched_text': matched_text
                                }
                                detection_method = "article_with_title"
                    except Exception as e:
                        self.logger.warning(f"带标题的文章查找失败: {e}")
                ...
                if coordinates:
                    ...
                    self.logger.warning(f"⚠️ 未找到目标新闻文章")
                    return None
    ...
    def calculate_all_coordinates(self) -> Dict[str, Any]:
        ...
        """
        self.logger.info(f"\n{'='*60}")
        self.logger.info(f"🚀 开始计算所有NPR HTML变体坐标")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='NPR coordinate calculation script')
    parser.add_argument('--input-dir', type=str, 
                       default='data/npr/html',
                       help='Directory containing HTML variants')
    parser.add_argument('--output-dir', type=str,
                       default='data/npr',
                       help='Output directory for coordinates and reports')
    ...
    # 创建坐标计算器
    calculator = NPRCoordinateCalculator(args.input_dir, args.output_dir)
