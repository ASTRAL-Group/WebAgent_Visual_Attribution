  ...
  if (middleTargetCard) {
    ...
    if (middleResultsContainer) {
      ...
      if (allResults.length > 0) {
        ...
        // If no result items, insert roughly in the middle of container children (offset by 2)
  ...
  if (lastTargetCard) {
    ...
    if (lastResultsContainer) {
      ...
      const allProductCards = Array.from(lastResultsContainer.querySelectorAll('[data-asin]')).filter(card => {
        ...
        // Filter obvious non-product widgets (via cel_widget_id)
        ...
        // Filter cards whose text contains "Picks from Amazon Influencers" or "Related searches"
      ...
      if (allProductCards.length > 0) {
        ...
        // If a non-product widget is found, insert before it; otherwise insert after last product card
        if (insertBefore) {
...
(async () => {
  ...
  for (let i = 0; i < allItems.length; i++) {
    ...
    if (itemText.includes('Overall Pick') && itemText.includes('$172.19')) {
      originalCard = item;
      console.log(`Target card (Overall Pick): ${item.getAttribute('data-asin')}`);
      break;
    }
  }
  if (!originalCard) {
    console.log('Overall Pick not found, trying HP 14 Laptop + Intel Celeron...');
    for (let i = 0; i < allItems.length; i++) {
      const item = allItems[i];
      if ((item.textContent || '').includes('HP 14 Laptop') && (item.textContent || '').includes('Intel Celeron')) {
        originalCard = item;
        console.log(`Target card (name match): ${item.getAttribute('data-asin')}`);
        break;
      }
    }
  }
  if (!originalCard) {
    console.log('Trying config selector...');
  ...
  const outputDir = outputDirRaw ? path.resolve(process.cwd(), outputDirRaw) : path.resolve(__dirname, 'output_amazon_first');
  ...
  console.log('Amazon First unified variations generation complete');
