...
// Hard-coded config values (originally from config_amazon.json)
const CONFIG = {
...
function adjustSidebarHeight(doc, lastVisibleProduct, maxProducts) {
  /**
   * Adjust sidebar: keep all filter categories up to and including "Features", hide the rest.
   */
  ...
  if (!sidebar) {
    console.log(`Sidebar not found, skipping adjustment`);
    return;
  }
  ...
  for (let i = 0; i < filterCategories.length; i++) {
    ...
    if (heading) {
      ...
      if (headingText.toLowerCase().includes('features') && 
        ...
        featuresIndex = i;
        console.log(`Found "Features" category at index ${featuresIndex}`);
  ...
  if (featuresIndex === -1) {
    console.log(`"Features" category not found, skipping hide operation`);
    return;
  }
  ...
  for (let i = featuresIndex + 1; i < filterCategories.length; i++) {
    ...
    // Get category heading for logging
    const heading = category.querySelector('[role="heading"] span, [role="heading"]');
    const headingText = heading ? heading.textContent.trim() : `Category ${i + 1}`;
    console.log(`Hiding category: "${headingText}"`);
  }
  
  console.log(`Sidebar adjusted: kept ${featuresIndex + 1} categories (including "Features"), hid ${hiddenCount} categories`);
}
...
(async () => {
  ...
  const outputDir = outputDirRaw
    ...
    : path.resolve(__dirname, '../../data/amazon_middle');
